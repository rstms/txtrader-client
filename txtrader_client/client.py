...
class Config():
  def __init__(self, label, config_dict=None):
...
class API():
  def __init__(self, server='localhost', config=None):
    self.server=server
    self.config = Config(server, config)
  ...
  def shutdown(self, reason:str):
    """Request server shutdown; post reason to logs"""
    return self._call_txtrader_api('shutdown', {'reason': reason})
  ...
  def query_bars(self, symbol, period, start, end):
    ...
    for label, value in (('start', start), ('end', end)):
      if type(value) == str:
        if not re.match('^\d{4}-\d{2}-\d{2} \d{2}:\d{2}(:00)*$', value):
  ...
  def market_order(self, account:str, route:str, symbol:str, quantity:int):
    ...
    return self._call_txtrader_api('market_order', {'account': account, 'route': route, 'symbol': symbol, 'quantity': quantity})

  def stage_market_order(self, account:str, tag:str, route:str, symbol:str, quantity:int):
    ...
    return self._call_txtrader_api('stage_market_order', {'account': account, 'tag': tag, 'route': route, 'symbol': symbol, 'quantity': quantity})

  def execute_staged_market_order(self, order_id: str):
    return self._call_txtrader_api('execute_staged_market_order', {'id': order_id})
  ...
  def limit_order(self, account: str, route: str, symbol: str, limit_price: float, quantity: int):
    ...
    return self._call_txtrader_api('limit_order', {'account': account, 'route': route, 'symbol': symbol, 'limit_price': float(limit_price), 'quantity': quantity})
  ...
  def stop_order(self, account: str, route: str, symbol: str, stop_price: float, quantity: int):
    ...
    return self._call_txtrader_api('stop_order', {'account': accunt, 'route': route, 'symbol': symbol, 'stop_price': stop_price, 'quantity': quantity})

  def stoplimit_order(self, account: str, route: str, symbol: str, stop_price: float, limit_price: float, quantity: int):
    """Submit a stop-limit order, returning dict containing new order fields"""
    return self._call_txtrader_api('stoplimit_order', {'account': account, 'route': route, 'symbol': symbol, 'stop_price': float(stop_price), 'limit_price': float(limit_price), 'quantity': int(quantity)})
